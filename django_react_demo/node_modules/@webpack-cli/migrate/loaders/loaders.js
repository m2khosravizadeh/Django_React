...
function default_1(j, ast) {
     ...
     */
    const createArrayExpressionFromArray = (path) => {
        const value = path.value;
        // Find paths with `loaders` keys in the given Object
        const paths = value.properties.filter((prop) => prop.key.name.startsWith("loader"));
        ...
        paths.forEach((pair) => {
            ...
            if (pair.value.type === j.ArrayExpression.name) {
                ...
                pair.value = j.arrayExpression(pairValue.elements.map((arrElement) => {
                ...
                pair.value = j.arrayExpression(literalValue.value.split("!").map((loader) => {
    ...
    const createLoaderWithQuery = (p) => {
        ...
        const loaderValue = properties.reduce((val, prop) => prop.key.name === "loader" ? prop.value.value : val, "");
        const loader = loaderValue.split("?")[0];
        const query = loaderValue.split("?")[1];
        const options = query.split("&").map((option) => {
        ...
        const loaderProp = utils.createProperty(j, "loader", loader);
        const queryProp = j.property("init", j.identifier("options"), j.objectExpression(options));
    ...
    const loadersToArrayExpression = () => ast
        ...
        .filter((path) => utils.safeTraverse(path, ["parent", "parent", "node", "key", "name"]) === "rules")
     ...
     * @returns {Node} ast - jscodeshift ast
     */
    const addLoaderSuffix = () => ast.find(j.ObjectExpression).forEach((path) => {
    ...
    const fitOptionsToUse = (p) => {
        ...
        if (options) {
            p.value.properties = p.value.properties.filter((prop) => prop.key.name !== "options");
            p.value.properties.forEach((prop) => {
                const keyName = prop.key.name;
                if (keyName === "use") {
