...
function replaceWithPath(j, p, pathVarName) {
    const convertedPath = j.callExpression(j.memberExpression(j.identifier(pathVarName), j.identifier("join"), false), [j.identifier("__dirname"), p.value]);
...
function default_1(j, ast) {
    ...
    if (literalOutputPath) {
        ...
        literalOutputPath.find(j.Literal).replaceWith((p) => replaceWithPath(j, p, finalPathName));
        if (!isPathPresent) {
